    ...
    try {
        ...
        if (arr || obj || func) {
            ...
            if (typeof Text != undefinedType && d instanceof Text) { r = 'Text' + (d.__text ? (' ' + readableStringify(d.__text, 1)) : '') } else
                if (typeof ENode != undefinedType && d instanceof ENode) { r = 'ENode' + (d.name ? (' ' + readableStringify(d.name, 1)) : '') } else
...
function makeSingleton(data, protoMethods, properties, baseClassProto, wrapMethods) {
    ...
    return new (makeClass(
        function () { if (baseClassProto) baseClassProto.apply(this, arguments); mergeObj(this, data); },
        protoMethods,
        properties,
        baseClassProto,
        wrapMethods
