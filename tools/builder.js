module.exports = (function () {
    ...
    var subtargetsBuilders = {
        ...
        atlas(d) {
            ...
            var useProxyFile = ifdef(d.useProxyFile, '.atlasProxy');

            if (useProxyFile) {
                fs.writeFileSync(useProxyFile, src, 'utf8');
                src = useProxyFile;
            }
            ...
            spawn(["bash"
                ...
                , d.npot ? '-2' : ''
                , d.nocrop ? '-c' : ''
                , '-u', ifdef(d.packerMethod, 2)
                , '-b', ifdef(d.border, 1)
                , '-n', ifdef(d.packerType, 0)
                , '-d', ifdef(d.dst, '.')
                , '-w', ifdef(d.width, 0)
                , '-h', ifdef(d.height, 0)
                , '-W', ifdef(d.maxWidth, 4096)
                , '-H', ifdef(d.maxHeight, 4096)
                , '-m', ifdef(d.mask, ifdef(d.name, "atlas") + '-%d'),
                , '-g', ifdef(d.format, 'short')
                , useProxyFile ? '-L' : '-l', quotesWrap(src)
            ]);

            if (!d.useProxyFile && useProxyFile) {
                removeFile(useProxyFile);
            }
