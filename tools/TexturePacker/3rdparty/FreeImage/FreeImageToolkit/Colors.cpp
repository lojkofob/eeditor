 ...
 */
int DLL_CALLCONV
FreeImage_GetAdjustColorsLookupTable(BYTE *LUT, double brightness, double contrast, double gamma, FIBOOL invert) {
	double dblLUT[256];
	double value;
	int result = 0;
...
FreeImage_ApplyColorMapping(FIBITMAP *dib, RGBQUAD *srccolors, RGBQUAD *dstcolors, unsigned count, FIBOOL ignore_alpha, FIBOOL swap) {
	unsigned result = 0;
 ...
 */
unsigned DLL_CALLCONV
FreeImage_SwapColors(FIBITMAP *dib, RGBQUAD *color_a, RGBQUAD *color_b, FIBOOL ignore_alpha) {
 ...
 */
unsigned DLL_CALLCONV
FreeImage_ApplyPaletteIndexMapping(FIBITMAP *dib, BYTE *srcindices,	BYTE *dstindices, unsigned count, FIBOOL swap) {
	unsigned result = 0;
