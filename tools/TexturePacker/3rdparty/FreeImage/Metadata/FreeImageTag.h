...
// JPEG Exif profile
FIBOOL jpeg_read_exif_profile(FIBITMAP *dib, const BYTE *dataptr, unsigned int datalen);

// JPEG / TIFF IPTC profile
FIBOOL read_iptc_profile(FIBITMAP *dib, const BYTE *dataptr, unsigned int datalen);
FIBOOL write_iptc_profile(FIBITMAP *dib, BYTE **profile, unsigned *profile_size);
