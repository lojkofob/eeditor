...
static BOOL_FreeImage rgbe_Error(rgbe_error_code error_code, const char *msg);
static BOOL_FreeImage rgbe_GetLine(FreeImageIO *io, fi_handle handle, char *buffer, int length);
static inline void rgbe_FloatToRGBE(BYTE rgbe[4], FIRGBF *rgbf);
static inline void rgbe_RGBEToFloat(FIRGBF *rgbf, BYTE rgbe[4]);
static BOOL_FreeImage rgbe_ReadHeader(FreeImageIO *io, fi_handle handle, unsigned *width, unsigned *height, rgbeHeaderInfo *header_info);
static BOOL_FreeImage rgbe_WriteHeader(FreeImageIO *io, fi_handle handle, unsigned width, unsigned height, rgbeHeaderInfo *info);
static BOOL_FreeImage rgbe_ReadPixels(FreeImageIO *io, fi_handle handle, FIRGBF *data, unsigned numpixels);
static BOOL_FreeImage rgbe_WritePixels(FreeImageIO *io, fi_handle handle, FIRGBF *data, unsigned numpixels);
static BOOL_FreeImage rgbe_ReadPixels_RLE(FreeImageIO *io, fi_handle handle, FIRGBF *data, int scanline_width, unsigned num_scanlines);
static BOOL_FreeImage rgbe_WriteBytes_RLE(FreeImageIO *io, fi_handle handle, BYTE *data, int numbytes);
static BOOL_FreeImage rgbe_WritePixels_RLE(FreeImageIO *io, fi_handle handle, FIRGBF *data, unsigned scanline_width, unsigned num_scanlines);
static BOOL_FreeImage rgbe_ReadMetadata(FIBITMAP *dib, rgbeHeaderInfo *header_info);
static BOOL_FreeImage rgbe_WriteMetadata(FIBITMAP *dib, rgbeHeaderInfo *header_info);
...
rgbe_ReadHeader(FreeImageIO *io, fi_handle handle, unsigned *width, unsigned *height, rgbeHeaderInfo *header_info) {
	...
	float tempf;
	int i;
	BOOL_FreeImage bFormatFound = FALSE;
	BOOL_FreeImage bHeaderFound = FALSE;
...
rgbe_WritePixels_RLE(FreeImageIO *io, fi_handle handle, FIRGBF *data, unsigned scanline_width, unsigned num_scanlines) {
	...
	while(num_scanlines-- > 0) {
		...
		for(int i = 0; i < 4; i++) {
			BOOL_FreeImage bOK = rgbe_WriteBytes_RLE(io, handle, &buffer[i*scanline_width], scanline_width);
			if(!bOK) {
				free(buffer);
				return bOK;
			}
...
static BOOL_FreeImage DLL_CALLCONV
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data) {
	...
	BOOL_FreeImage header_only = (flags & FIF_LOAD_NOPIXELS) == FIF_LOAD_NOPIXELS;
