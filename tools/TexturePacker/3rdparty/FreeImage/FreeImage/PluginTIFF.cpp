...
// TIFF Exif profile
BOOL_FreeImage tiff_read_exif_tags(TIFF *tif, TagLib::MDMODEL md_model, FIBITMAP *dib);
BOOL_FreeImage tiff_write_exif_tags(TIFF *tif, TagLib::MDMODEL md_model, FIBITMAP *dib);
...
static FIBITMAP* CreateImageType(BOOL_FreeImage header_only, FREE_IMAGE_TYPE fit, int width, int height, uint16 bitspersample, uint16 samplesperpixel);
...
static BOOL_FreeImage tiff_read_iptc_profile(TIFF *tiff, FIBITMAP *dib);
static BOOL_FreeImage tiff_read_xmp_profile(TIFF *tiff, FIBITMAP *dib);
static BOOL_FreeImage tiff_read_exif_profile(TIFF *tiff, FIBITMAP *dib);
static void ReadMetadata(TIFF *tiff, FIBITMAP *dib);

static BOOL_FreeImage tiff_write_iptc_profile(TIFF *tiff, FIBITMAP *dib);
static BOOL_FreeImage tiff_write_xmp_profile(TIFF *tiff, FIBITMAP *dib);
...
*/
static FIBITMAP* 
CreateImageType(BOOL_FreeImage header_only, FREE_IMAGE_TYPE fit, int width, int height, uint16 bitspersample, uint16 samplesperpixel) {
	FIBITMAP *dib = NULL;
...
Open(FreeImageIO *io, fi_handle handle, BOOL_FreeImage read) {
...
FindLoadMethod(TIFF *tif, FREE_IMAGE_TYPE image_type, int flags) {
	...
	BOOL_FreeImage bIsTiled = (TIFFIsTiled(tif) == 0) ? FALSE:TRUE;
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data) {
	...
	const BOOL_FreeImage header_only = (flags & FIF_LOAD_NOPIXELS) == FIF_LOAD_NOPIXELS;
	...
	try {	
		...
		const BOOL_FreeImage asCMYK = (flags & TIFF_CMYK) == TIFF_CMYK;
		...
		if(loadMethod == LoadAsRBGA) {
			...
			BOOL_FreeImage has_alpha = FALSE;   
			...
			BOOL_FreeImage isCMYKA = (photometric == PHOTOMETRIC_SEPARATED) && (samplesperpixel > 4);
			...
			if(!header_only) {
				...
				BOOL_FreeImage bThrowMessage = FALSE;
...
Save(FreeImageIO *io, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data) {
	...
	const BOOL_FreeImage bHasThumbnail = (FreeImage_GetThumbnail(dib) != NULL);
