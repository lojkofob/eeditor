...
public:
	...
	BOOL_FreeImage isNull() const { 
		return (_data == NULL); 
	}
	
	BOOL_FreeImage read(FreeImageIO *io, fi_handle handle, size_t size) {
...
public:
	...
	BOOL_FreeImage isNull() { return _begin == NULL;}
...
*/
static void 
loadTrueColor(FIBITMAP* dib, int width, int height, int file_pixel_size, FreeImageIO* io, fi_handle handle, BOOL_FreeImage as24bit) {
...
template <int nBITS>
inline static void 
_assignPixel(BYTE* bits, BYTE* val, BOOL_FreeImage as24bit = FALSE) {
...
template <>
inline void 
_assignPixel<8>(BYTE* bits, BYTE* val, BOOL_FreeImage as24bit) {
	*bits = *val;
}

template <>
inline void 
_assignPixel<16>(BYTE* bits, BYTE* val, BOOL_FreeImage as24bit) {
...
template <>
inline void 
_assignPixel<24>(BYTE* bits, BYTE* val, BOOL_FreeImage as24bit) {
...
template <>
inline void 
_assignPixel<32>(BYTE* bits, BYTE* val, BOOL_FreeImage as24bit) {
	if (as24bit) {
...
template<int bPP>
static void 
loadRLE(FIBITMAP* dib, int width, int height, FreeImageIO* io, fi_handle handle, long eof, BOOL_FreeImage as24bit) {
	...
	while (y < height) {
		...
		BOOL_FreeImage has_rle = rle & 0x80;
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data) {
	...
	try {
		...
		if(isTARGA20(io, handle)) {
			...
			SwapFooter(&footer);
#endif
			BOOL_FreeImage hasExtensionArea = footer.extension_offset > 0;
			if(hasExtensionArea) { 
				...
				BOOL_FreeImage hasThumbnail = (postage_stamp_offset > 0) && (postage_stamp_offset < (DWORD)footer_offset);
				if(hasThumbnail) {
...
*/
static inline void 
flushPacket(BYTE*& dest, unsigned pixel_size, BYTE* packet_begin, BYTE*& packet, BYTE& packet_count, BOOL_FreeImage& has_rle) {
	if (packet_count) {
