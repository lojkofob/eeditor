...
PluginList::PluginList() : m_plugin_map(),
						   m_node_count(0)
{
}
...
FreeImage_GetFIFExtensionList(FREE_IMAGE_FORMAT fif)
{
	if (s_plugins != NULL)
	{
		...
		return (node != NULL) ? (node->m_extension != NULL) ? node->m_extension : (node->m_plugin->extension_proc != NULL) ? node->m_plugin->extension_proc()
																														   : NULL
							  : NULL;
	}
...
FreeImage_GetFIFDescription(FREE_IMAGE_FORMAT fif)
{
	if (s_plugins != NULL)
	{
		...
		return (node != NULL) ? (node->m_description != NULL) ? node->m_description : (node->m_plugin->description_proc != NULL) ? node->m_plugin->description_proc()
																																 : NULL
							  : NULL;
	}
...
FreeImage_GetFIFRegExpr(FREE_IMAGE_FORMAT fif)
{
	if (s_plugins != NULL)
	{
		...
		return (node != NULL) ? (node->m_regexpr != NULL) ? node->m_regexpr : (node->m_plugin->regexpr_proc != NULL) ? node->m_plugin->regexpr_proc()
																													 : NULL
							  : NULL;
	}
...
FreeImage_FIFSupportsExportBPP(FREE_IMAGE_FORMAT fif, int depth)
{
	if (s_plugins != NULL)
	{
		...
		return (node != NULL) ? (node->m_plugin->supports_export_bpp_proc != NULL) ? node->m_plugin->supports_export_bpp_proc(depth) : FALSE : FALSE;
	}
...
FreeImage_FIFSupportsExportType(FREE_IMAGE_FORMAT fif, FREE_IMAGE_TYPE type)
{
	if (s_plugins != NULL)
	{
		...
		return (node != NULL) ? (node->m_plugin->supports_export_type_proc != NULL) ? node->m_plugin->supports_export_type_proc(type) : FALSE : FALSE;
	}
...
FreeImage_FIFSupportsICCProfiles(FREE_IMAGE_FORMAT fif)
{
	if (s_plugins != NULL)
	{
		...
		return (node != NULL) ? (node->m_plugin->supports_icc_profiles_proc != NULL) ? node->m_plugin->supports_icc_profiles_proc() : FALSE : FALSE;
	}
...
FreeImage_FIFSupportsNoPixels(FREE_IMAGE_FORMAT fif)
{
	if (s_plugins != NULL)
	{
		...
		return (node != NULL) ? (node->m_plugin->supports_no_pixels_proc != NULL) ? node->m_plugin->supports_no_pixels_proc() : FALSE : FALSE;
	}
