...
(function (root, factory) {
    ...
    function compileAST(ast, opts, anno) {
        options = opts || {};
        annotations = anno || {};
        externFuncs = { collectgarbage: 1 };
        if (options.externFuncs) {
            var ar = options.externFuncs.split(',');
    ...
    function compileForInStatement(statement, compiledLabel) {
        ...
        // Label for continue
//         console.log("Warning: ::_continue:: skipped in ForInStatement");
...
//         throw new Error("WhileStatement not supported cause of ::_continue::.");
...
//         throw new Error("DoWhileStatement not supported cause of ::_continue::.");
    ...
    function compileExpression(expression, meta) {
        ...
        case "ArrowFunctionExpression":
            return compileFunction(expression, meta, 1);
    ...
    function compileCallExpression(expression, meta) {
        ...
        if (expression.callee.type === "MemberExpression") {
            ...
            if (compiledCallee.match(/\]$/)) {
                // console.log('expression.callee', expression.callee);
                // console.log('compiledCallee', compiledCallee);
                var startIndex = lastTopLevelBracketedGroupStartIndex(compiledCallee);
                var base = compiledCallee.substr(0, startIndex);
                
                if (shortFuncs) { // над этим еще надо подумать
                    
                    // @number
                    var member = compiledCallee.substr(startIndex + 1, compiledCallee.length - 2 - startIndex );
                    
                    console.log('compiledCallee', compiledCallee, member);
                    
                    if (member) {
                        compiledCallExpression.push(base + ':' + member + '(');
                    } else {
            ...
            compiledCallExpression.push(compiledCallee);
            var skipZpt;
            if (compiledCallee.length < 30 && externFuncs[compiledCallee]) {
                compiledCallExpression.push('(');
                skipZpt = 1;
            }
            else
    ...
    function compileObjectExpression(expression, meta) {
        ...
        for (i = 0; i < length; ++i) {
            ...
            if (property.key.type === "Literal") {
                ...
                if ( shortKeys && compiledKey.match(/^\w[\d\w]*$/g)
                    && luaKeywords.indexOf(compiledKey) < 0) {
                        compiledKeySimple = 1;
                } else {
                    // @string
            ...
            if (compiledKeySimple) {
                compiledProperty = [ compiledKey, " = "];
            } else {
                compiledProperty = ["[", compiledKey, "] = "];
            }
    ...
    function compileMemberExpression(expression, meta) {
        ...
        if (expression.computed) {
            ...
            if (compiledProperty !== expression.property.name) {
                ...
                compiledProperty = sanitizeLiteralString(expression.property.name);
                
                var compiledKeySimple = shortKeys && compiledProperty.match(/^\w[\d\w]*$/g)
                    && luaKeywords.indexOf(compiledProperty) < 0;
    ...
    function compileVariableDeclaration(variableDeclaration) {
        ...
        case "var":
            ...
            var i, declarator, pattern, expression;
            ...
            for (i = 0; i < declarations.length; ++i) {
                ...
                if (declarator.init !== null) {
                    ...
                    compiledDeclarations.push(pattern + " = " + expression + ";");
                }
            }
    ...
    function compileFunction(fun, meta, arrow) {
        ...
        if (fun.body.type === "BlockStatement") {
            ...
            if (arrow) {
                compiledBody = "return " + compileExpression(fun.body);
            } else {
                throw new Error("unknown function body type " + fun.body.type);
            }
        }
