...
TreeEntry.prototype = {
    ...
    add: function( newentry, noselect){
        ...
        if (entry.treeItem){
            ...
            newentry.parentNode = entry.treeItem.content;
    ...
    remove: function(  ){
        ...
        if (entry.parentEntry) {
            removeFromArray( this, entry.parentEntry.data );
    ...
    onDrag: function(x,y,dx,dy){
        ...
        if (pl) {
            ...
            var wp = pl.__worldPosition.clone();
            wp.z = -1000;
    ...
    onDragStart: function(x,y,dx,dy){
        ...
        var pl = this.treeItem ?  this.treeItem.plate : 0;
        if (pl) {
            pl.__sragStartOfs = pl.__ofs.clone();
            pl.__z -= 30;
        ...
        if (this.treeItem) {
            ...
            if ( params.withAutoScroll && pl){
                if (pl.__scrollIntoView) {
                    if (this.parentNode && this.parentNode.__root) this.parentNode.__root.update(1);
    ...
    updateSelfIndex: function(newindex, insertVariant){
        if (this.treeItem.parent.border)
            newindex++;
        
        if (insertVariant){
            
            removeFromArray( this.treeItem , this.treeItem.parent.__childs);
            
            this.treeItem.parent.__childs.splice(newindex, 0, this.treeItem);
    ...
    updateItem: function( parentDeep, childDeep ){
        ...
        if (!entry.__isRoot) {
            if (!item && entry.parentNode) {
                
                item = entry.treeItem =
                entry.parentNode.__addChildBox( TreeWithKitten.options.itemTemplate );
            ...
            if (entry.icon){
                ...
                item.plate.icon.__img = entry.icon;
                x += 30;
            } else {
                if (item.plate.icon) {
            ...
            if (entry.text) {
                ...
                item.plate.txt.__text = entry.text;
                item.plate.txt.__x = x;
                item.name = 'i_' + entry.text;
            } else {
                if (item.plate.txt) {
        ...
        if (parentDeep){
            if ( entry.parentEntry )
                entry.parentEntry.updateItem(1);
        }
