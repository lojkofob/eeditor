...
//                     },
                    ...
                    __childs: {
                        __border: {
                            ...
                            __updateVertices: updateVerticesCustomFunction( function(){
                                var s = node.__size.clone().__multiplyScalar(0.5);
                                return [ 
                                    -s.x, s.y,
                                    -s.x, -s.y,
                                    s.x, -s.y,
                    ...
                    __render: function(){
                        ...
                        if (node.__worldPosition.x!=this.wpppp.x||node.__worldPosition.y!=this.wpppp.y){
                            this.__needUpdate = 1;
                            this.wpppp.copy(node.__worldPosition);
                        }
...
//                     __geometryOffset:{ get: function() { return node.__geometryOffset } },
                    __size: createSomePropertyWithGetterAndSetter( function() {
                        ...
                        if (this.____szzz) {
                            this.____szzz.copy(node.__size);
                        }
                        else {
                            this.____szzz = node.__size.clone();
                        }
                            ...
                            __drag: function(_x,_y,_dx,_dy){
                                ...
                                if (isShiftPressed){
                                    ...
                                    if (node.sha == undefined) {
                                        if ( node.parent.ha != undefined && node.parent.ha != ALIGN_CENTER ) {
                                            xmult = 1;
                                            
                                            if ( node.parent.ha == ALIGN_START ||  node.parent.ha == ALIGN_FROM_START_TO_END ) if (x == -1) xxx = 0;
                                            if ( node.parent.ha == ALIGN_END ||  node.parent.ha == ALIGN_FROM_END_TO_START ) if (x == 1) xxx = 0;
                                    ...
                                    if (node.sva == undefined) {
                                        if ( node.parent.va != undefined && node.parent.va != ALIGN_CENTER ) {
                                            ymult = 1;
                                        
                                            if ( node.parent.va == ALIGN_START ||  node.parent.va == ALIGN_FROM_START_TO_END ) if (y == -1) yyy = 0;
                                            if ( node.parent.va == ALIGN_END ||  node.parent.va == ALIGN_FROM_END_TO_START ) if (y == 1) yyy = 0;
                                        }
