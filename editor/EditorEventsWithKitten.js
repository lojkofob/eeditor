...
function offsetChanger(dx, dy, dz){
	...
	return function(node) {
        var ofs = node.__offset.clone(),
            v = new Vector3(dx, dy, 0),
            mw = node.parent.__matrixWorld.__getInverseMatrix().__scale(new Vector3(1,-1, 1));
        ...
        if (Editor.currentLayout) {
            eachSelected( function(selectedNode){
                if (!selectedNode.__someParentSelected()) {
                    changes.push( { type:'-', node: selectedNode, parent: selectedNode.parent, index: selectedNode.__realIndex } );
            ...
            if (p && p != Editor.currentLayout.layoutView && p.treeEntry && p.parent )
                p.__select();
...
//             if (!hasSelected){
                looperPost( a=> {
                    if (!curDraggingObject) {
                        QuadSelectorWithKitten.activate(Editor.currentLayout.layoutView.$( n => n.__validToSave && n.__selectable ), isAltPressed ? 0 : 1);
                    }
                } );
                return true;
//             }
...
function __selectChildTraversing(f){
    ...
    function kk(node) {
        node.__traverseChildsFilter(
            ...
            n => n.__validToSave && n.__selectable
        );
    }
    if (isAltPressed){
        eachSelected(kk);
            ...
            for ( var y = -80*z; y<80*z; y+=10) {
                var ppos = new Vector2(pos.x + x, pos.y + y)
                    ...
                    , ddebug = __window.ddd.__addChildBox({ 
                        __ofs: screenpos.clone() , __size: {x:5, y:5}, __notSelectable:1, __disabled:1, __validToSave: 0,
...
function enterFullScreen(){
    var elem = renderer.__domElement;
